...
BATCH_E_DIR = ROOT / "research" / "batches" / "batch_e_discourse_and_dialogue"
BATCH_E_KEYS: tuple[str, ...] = ("22", "23", "24", "25")
...
def _load_chapter_module(key: str):
    return importlib.import_module(_module_name_for_key(key))


def build_batch_e_payload() -> dict[str, object]:
    chapter_map = get_chapter_map()
    chapters: list[dict[str, object]] = []
    for key in BATCH_E_KEYS:
        spec = chapter_map[key]
        module = _load_chapter_module(key)
        ...
        chapters.append(
            {
                ...
                "eval_pack": {
                    ...
                    "sources": _to_jsonable(payload["sources"]),
                },
            }
        )
    return {
        ...
        "chapters": [
            {
                ...
                "fixture_path": f"research/batches/batch_e_discourse_and_dialogue/fixtures/{chapter['fixture_filename']}",
                "eval_pack_path": f"research/batches/batch_e_discourse_and_dialogue/eval_packs/{chapter['eval_pack_filename']}",
            }
...
def write_batch_e_artifacts(base_dir: Path = BATCH_E_DIR) -> Path:
    ...
    manifest_path = base_dir / "BATCH_E_MANIFEST.json"
